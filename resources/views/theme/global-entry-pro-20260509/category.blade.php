    ...
    <div class="ne-shell ne-layout">
        <section class="ne-feed">
            <div class="ne-page-head ne-category-head">
                ...
                @endif
                <div class="ne-category-service-row">
                    <span>栏目定位</span>
                    <strong>围绕入驻条件、资料准备、审核避坑和运营起步持续更新。</strong>
                </div>
                <div class="ne-category-tabs" aria-label="{{ __('front.nav.categories') }}">
                    @foreach((isset($navCategories) ? collect($navCategories) : collect([$category]))->reject(fn ($item) => $item->slug === 'geo-ai-search') as $categoryItem)
        ...
        @include('theme.global-entry-pro-20260509.partials.sidebar')
    </div>
@endsection
